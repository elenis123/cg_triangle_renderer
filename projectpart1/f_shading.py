...
def f_shading(img, vertices, vcolors):
    # Function for shading a polygon defined by its vertices on an image
    color = np.mean(vcolors, axis=0)# Get average color of the triangle's vertices
    img=list(img)
    ...
    #list of active points in each scan line and upadate them at every scan line
    for scan in range(fisrt_scan,last_scan+1):
        ...
        if len(active_points)==1:
            ...
            for i in range(min_active_points, max_active_points):
              if [i ,scan] in active_points:
                   continue
              else:
                   img[i][scan]= list(color) #color the pixel 
